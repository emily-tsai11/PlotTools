...
if __name__ == '__main__':
    ...
    if '-d' in opts_unknown:
       ...
       KILL(log_prx+'option "-d" not provided (required by submitImpacts, add it before the path to the datacard)')
    ...
    if '-m' in opts_unknown:
       ...
       KILL(log_prx+'option "-m" not provided (required by submitImpacts)')
    ...
    if VERBOSE: print ('\n'+'-'*3+' submitImpacts', '[NAME='+OPT_NAME+'] [MASS='+OPT_MASS+']'+'\n')
    ...
    if opts_unknown[dcard_idx].endswith('.root'):
      ...
      #  COMBINE_OPTS_fits = COMBINE_OPTS

      #  if "_VR" in OUT_DIR or "_CR" in OUT_DIR:
    #    if "_CR" in OUT_DIR:
       if "_CR" in OUT_DIR and not "_SR" in OUT_DIR:
        output_dict = {
            ...
            'shell_commands': [
                ...
                # [ 'cd '+OUT_DIR ],
                # [ 'cd '+OUT_DIR_EOS ],
                ['combineTool.py'] + COMBINE_OPTS+['-M '+OPT_MODE, '--doInitialFit --robustFit 1 --saveFitResult --saveWorkspace --saveNLL --X-rtd REMOVE_CONSTANT_ZERO_POINT=1']             + ['&> '+OFILE_NAME+'.doInitialFit.out.txt'],
                # ['combineTool.py'] + COMBINE_OPTS+['-M '+OPT_MODE, '--doInitialFit --saveFitResult --saveWorkspace --saveNLL --X-rtd REMOVE_CONSTANT_ZERO_POINT=1']             + ['&> '+OFILE_NAME+'.doInitialFit.out.txt'],
                # ['combineTool.py'] + COMBINE_OPTS+['-M '+OPT_MODE, '--doFits', '--parallel 4'] + ['&> '+OFILE_NAME+'.doFits.out.txt'],
                ['combineTool.py'] + COMBINE_OPTS+['-M '+OPT_MODE, '--doFits --robustFit 1', '--parallel 8'] + ['&> '+OFILE_NAME+'.doFits.out.txt'],
            ...
            #     ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttcc'] + ['--POI xsec_ttcc'],
            #     ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttcj'] + ['--POI xsec_ttcj'],
            #     ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttbb'] + ['--POI xsec_ttbb'],
            #     ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttbj'] + ['--POI xsec_ttbj'],
            #     ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_tt2b'] + ['--POI xsec_tt2b'],
            #     ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_tt2c'] + ['--POI xsec_tt2c'],
            #     ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttLF'] + ['--POI xsec_ttLF'],
            #     [ 'cp *.root *.txt *.pdf *.json '+OUT_DIR_EOS+'/' ],
            #     [ 'rm -rf *.root *.txt *.pdf *.json' ],
        ...
        if "CRbb" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttbb'] + ['--POI xsec_ttbb'],
            ]
        elif "CRbj" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttbj'] + ['--POI xsec_ttbj'],
            ]
        elif "CRcc" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttcc'] + ['--POI xsec_ttcc'],
            ]
        elif "CRcj" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttcj'] + ['--POI xsec_ttcj'],
            ]
        elif "CR2b" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_tt2b'] + ['--POI xsec_tt2b'],
            ]
        elif "CR2c" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_tt2c'] + ['--POI xsec_tt2c'],
            ]
        elif "CRlf" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttLF'] + ['--POI xsec_ttLF'],
            ]
        elif "CRnolf" in OUT_DIR:
            output_dict['shell_commands'] += [
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttcc'] + ['--POI xsec_ttcc'],
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttcj'] + ['--POI xsec_ttcj'],
                ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_xsec_ttbb'] + ['--POI xsec_ttbb'],
        ...
        output_dict['shell_commands'] += [
         ...
         'shell_commands': [
            ...
            ['combineTool.py'] + COMBINE_OPTS+['-M '+OPT_MODE, '-o '+OFILE_NAME+'.json']    + ['&> '+OFILE_NAME+'.json.out.txt'],
         #    ['plotImpacts.py'] + ['--blind'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME],
            ['plotImpacts.py'] + ['-i '+OFILE_NAME+'.json'] + ['-o '+OFILE_NAME+'_r'] + ['--POI r'],
            [ 'cp *.root *.txt *.pdf *.json '+OUT_DIR_EOS+'/' ],
            [ 'rm -rf *.root *.txt *.pdf *.json' ],
            # ['touch '+OFILE_NAME+'.completed'],
            ['touch '+OUT_DIR+"/"+OFILE_NAME+'.completed'],
           ...
           if opts.batch == 'htc':
              ...
              batch_job_HTCondor(**{
               ...
               #  'is_slc7_arch': is_slc7_arch,
                ...
                'longjob': ('Impacts' in i_output_basename or 'GoodnessOfFit' in i_output_basename),
              })
