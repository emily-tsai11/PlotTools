...
Systematics-batched variant of hdumper.py, built on RDataFrame's Vary() /
VariationsFor() API (ROOT::RDF::Experimental).

Why this exists: hdumper.py computes each systematic's weight via its own
separate Define() call -- ~150 systematics per selection means ~150 distinct
strings individually JIT-compiled by cling, even though they all boil down to
"multiply the nominal weight by one more ratio branch". Vary() lets you
express "one nominal value + N alternate values" as a SINGLE expression (one
JIT-compiled function returning a vector of all N+1 values), and
VariationsFor() unpacks the nominal result plus every variation from ONE
booked action. Concretely, per selection this collapses roughly
...
Benchmarked against the literal old pattern (N separate Define+Histo1D calls
vs. one Vary()+Histo1D()+VariationsFor()) on a synthetic tree: ~2.3x faster,
identical numeric results. See the module docstring notes below for a caveat
on VariationsFor()'s own laziness.

IMPORTANT gotcha this file works around: just like .GetValue()/.GetPtr(),
calling VariationsFor() triggers immediate execution of every action booked
so far on that RLoopManager. Calling it once per selection *inside* the
selection loop (the natural-looking place to put it) silently reintroduces
the exact "eager materialization inside a loop" bug that was fixed in
hdumper.py -- confirmed by direct benchmark (2x slower when called eagerly
per-selection vs. batched once at the end). So, as in hdumper.py, every
Histo1D/Sum/Count action below is only *booked* inside the selection loop;
VariationsFor() is called exactly once per booking, all together, in the
final "Materializing" block.

Everything else (CLI, selections, output file naming/merging, the weight
formulas themselves, the exact set of histograms and files produced) is
unchanged from hdumper.py -- this file should produce byte-for-byte
equivalent histograms, just computed with far fewer JIT-compiled functions.
...
def _active_systematics(systematics, do_systematics, infile):
    ...
    "None", the nominal case, which is always handled separately)."""
...
def process_tree(infile, outfile, tree_name, hist_configs, year, selections, eventClassification, use5FS, count_events, do_systematics=False):
    ...
    if "singlee" in infile:
        base_filter += " && passTrigMu==0"  # Remove from the electron channel the events that fired the muon trigger.
    ...
    for selection_name in selections:
        ...
        # branch) -- same hoisting as hdumper.py's fix, for the same reason.
        ...
        if not is_data:
            nominal_weight = assign_event_weight(year, infile, suffix)
        ...
        if active_systs:
            if is_data:
                ...
                vary_expr = "ROOT::RVecD{" + ",".join(
                    assign_event_weight(year, infile, suffix, systematics[syst]) for syst in active_systs
                ) + "}"
        ...
        # needs its own JIT compile of the same ~200-entry expression --
        # confirmed by direct benchmark to cost ~1.5x for no benefit.
...
def assign_event_weight(year, infile, suffix, syst=""):
    ...
    if "ttbar" in infile or "tt-vcb" in infile:
        weight = f"{weight}*TopPtWeight[1]*TopPtWeightNorm{suffix}[1]*TOPMLWeight[5]*TOPMLWeightNorm{suffix}[5]"
    if "4f" in infile:
        weight = f"{weight}*TopPtWeight[1]*TopPtWeightNorm{suffix}[1]*TOPMLWeight[5]*TOPMLWeightNorm{suffix}[5]"
...
def produce_systematics(year, suffix):
    ...
    systematics = {"None" : "",
               ...
               "bFragWeight_%sUp"   % year : f"(TOPMLWeight[4]*TOPMLWeightNorm{suffix}[4])/(TOPMLWeight[5]*TOPMLWeightNorm{suffix}[5])",
               "bFragWeight_%sDown" % year : f"1/(TOPMLWeight[5]*TOPMLWeightNorm{suffix}[5])",
               "bFragPetersonWeight_%sUp"   % year : f"(bFragAndDecayWeight[3]*BFragAndDecayWeightNorm{suffix}[3])/(TOPMLWeight[5]*TOPMLWeightNorm{suffix}[5])",
               "bFragPetersonWeight_%sDown" % year : f"1.",
               # LHE for large bkgs
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process ROOT TTrees into TH1D histograms (Vary()-batched systematics).")
